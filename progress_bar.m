function progressbar(fractiondone, description_string, position)
% Description:
...
%
% Modified by Ken to accept 'description_string'
...
if nargin < 3
% Modified by Ken for lower left
    position = 3;
end
...
if isempty(progfig)
    ...
    progfig = figure(...
        ...
        'MenuBar',          'none',...
        'BackingStore',     'off', ...
        'IntegerHandle','off' );	% Ken modification
    progaxes = axes(...
...
if etime(clock,lastupdate) < 0.1 % Modified from 0.01 by Ken
    return
end
...
if (fractiondone == 0)
    ...
    titlebarstr = sprintf('%s    %2d%%    %s remaining',description_string,percentdone,timeleftstr);	
end
...
% Modified by Ken to be a constant color
set(progpatch,'FaceColor',[0 0 1]);
