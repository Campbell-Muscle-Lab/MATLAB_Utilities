function out = superplot_w_mean_lines(params)
...
arguments
    ...
    params.link_line_back = 0;
    params.link_line_color = [0 0 0 0.5]
    params.x_tick_length = 0.05;
    params.x_end_cap = 0.5;
    params.mean_line_breadth=0.7;
    params.mean_line_width=2;
    params.mean_line_color=[0 0 0 0.5];
    params.mean_line_back=1;
end
...
for level_counter = 1:2
    ...
    % if numel(params.data_table.(params.grouping_variable)) == numel(unique_grouping_variables)
    %     params.raw_face_alpha = 0;
    %     params.raw_edge_color = [1 1 1]
    % end
    ...
    for i = 1 : max([1 numel(params.factor_1_strings)])
        ...
        for j=1:numel(params.factor_2_strings)
            ...
            % % Draw mean lines
            if (params.mean_line_width>0) && level_counter > 1 
                mean_y(mean_y==0) = [];
                mean_y = mean(mean_y);
                    m_l = plot(x_c+params.mean_line_breadth*[-0.5 0.5], ...
                        mean_y*[1 1],'-', ...
                        'Color',params.mean_line_color, ...
                        'LineWidth',params.mean_line_width);
                    mean_y = [];
                    
                    if params.mean_line_back
                        uistack(m_l,'bottom')
                    end
            end
        ...
        if (params.link_line_width > 0)
            for k=1:numel(unique_grouping_variables)
                link_line = plot(x_link(k,:),y_link(k,:),'Color',params.link_line_color, ...
                    'LineWidth', params.link_line_width);
                if params.link_line_back
                    uistack(link_line,'bottom')
                end
            end
        end
